...
const HomePage = () => {
  ...
  const [activeComponent, setActiveComponent] = useState("userStats");

  // ✅ Use environment variable for API base URL
  const API_BASE_URL = process.env.REACT_APP_API_URL || '';

  const handlePicksModalClick = () => setPicksModalOpen(true);
  const handleClosePicksModal = () => setPicksModalOpen(false);
  ...
  useEffect(() => {
    ...
    const fetchUserStats = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/picks/user/${currentUserId}/picks-and-stats`);
  ...
  useEffect(() => {
    ...
    const fetchMyStats = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/mystats/${currentUserId}`);
        ...
        <>
          ...
          <div className="logo-button-container">
            ...
            <div className="button-container">
              <button className="home-page-button" onClick={handleLogin}>Log In</button>
              <button className="home-page-button" onClick={() => setIsModalOpen(true)}>Sign Up</button>
            </div>
          </div>
        </>
      );
    ...
    switch (activeComponent) {
      case "userStats":
        return picksSubmitted
          ...
          : <button onClick={handlePicksModalClick} className="glowing-button">Make Picks</button>;
      case "leaderboard":
        return <Leaderboard />;
      case "leagues": // ✅ New case
      return <LeaguesPage />; // Import this at the top of the file
      default:
        return null;
    }
  };
    ...
    <div className="HomePage">
      ...
      {isLogInModalOpen && (
        <LogInModal
          ...
          switchToSignUp={() => { setIsLogInModalOpen(false); setIsModalOpen(true); }}
